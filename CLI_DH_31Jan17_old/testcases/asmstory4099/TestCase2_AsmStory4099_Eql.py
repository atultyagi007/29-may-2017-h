...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 FX servers with equallogic volume � one(5.1) and other(5.5) ,1 cluster and 1 VM after Scale up an server
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_Testcase2_AsmStory4099_Eql)
        self.getResources()
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.test_publishTemplate(serverRefID3)
    ...
    def test_publishTemplate(self,serverRefID3):
        logger = self.getLoggerInstance()
        ...
        statausDeploy = False
        DeplName='TestCase2_AsmStory4099_Eql'
        deplyDesc='Deploy 2 FX servers with equallogic volume one 5.1 and other 5.5 ,1 cluster and 1 VM after Scale up an server'
        ...
        if deployResponse.status_code in (200, 201, 202, 203, 204):            
            ...
            self.test_scaleUpTemplate(globalVars.publishedTemplateID, deploymentRefId, DeplName, deplyDesc,serverRefID3)
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc,serverRefID3):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_Testcase2_AsmStory4099_Eql_scaleup)
        payload = payload.replace("$serversource",inputReqValueESXI.serversource)
        if (inputReqValueESXI.serversource == "manual"):
            payload = payload.replace("$Server3_Entry",serverRefID3)
