...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    '''
        Deploy 2 servers with equallogic volume � one 12G blade (5.1) and other 12G rack (5.5) ,1 cluster and 1 VM after Scale up 13G rack (5.1) and 13G blade (5.5)
    ''' 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_Testcase1_AsmStory4099_Eql)
        self.getResources()
        ...
        payload = payload.replace("$serversource",'manual').replace("$Server1_Entry",serverRefID1).replace("$Server2_Entry",serverRefID2)
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.test_publishTemplate(serverRefID3,serverRefID4)
    ...
    def test_publishTemplate(self,serverRefID3,serverRefID4):
        logger = self.getLoggerInstance()
        ...
        statausDeploy = False
        DeplName='TestCase1_AsmStory4099_Eql'
        deplyDesc='Deploy 2 servers with equallogic volume one 12G blade 5.1 and other 12G rack 5.5 ,1 cluster and 1 VM after Scale up 13G rack 5.1 and 13G blade 5.5'
        ...
        if deployResponse.status_code in (200, 201, 202, 203, 204):            
            ...
            self.test_scaleUpTemplate(globalVars.publishedTemplateID, deploymentRefId, DeplName, deplyDesc,serverRefID3, serverRefID4)
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc,serverRefID3,serverRefID4):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_Testcase1_AsmStory4099_Eql_scaleup)
        payload = payload.replace("$serversource",inputReqValueESXI.serversource)
        if (inputReqValueESXI.serversource == "manual"):
            payload = payload.replace("$Server3_Entry",serverRefID3).replace("$Server4_Entry",serverRefID4)
