...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 FX2 server with Compellent volume – one 5.1 and other 5.5 ,1 cluster and 1 VM after Scale up an server
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase4_AsmStory4099_Comp)
        self.getResources()
        ...
        payload = payload.replace("$serversource",'manual').replace("$Server1_Entry",serverRefID1).replace("$Server2_Entry",serverRefID2).replace("$Server3_Entry",serverRefID3)
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase4_AsmStory4099_Comp'
        deplyDesc='Deploy 2 FX2 server with equallogic volume – one 5.1 and other 5.5 ,1 cluster and 1 VM after Scale up an server'
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase4_AsmStory4099_Comp_Scaleup)
