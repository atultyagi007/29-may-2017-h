...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 blade servers using ESX 5.5 with 1 compellent volume to an Exiting cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_104061)
        self.getResources()
        networkType=inputForNetworkType.TestCase_104061_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_104061, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_104061, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104061'
        deplyDesc=testCaseDescriptionMapping.TestCase_104061
