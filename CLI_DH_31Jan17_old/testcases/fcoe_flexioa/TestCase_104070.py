...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 blade servers using ESX 5.1 with 1 compellent volume to an Exiting cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_before_104070_scaleup_compellentstorage)
        self.getResources()
        networkType=inputForNetworkType.TestCase_104070_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104070, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104070, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104070'
        deplyDesc=testCaseDescriptionMapping.TestCase_104070
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_104070_after_scaleup_compellentstorage)
