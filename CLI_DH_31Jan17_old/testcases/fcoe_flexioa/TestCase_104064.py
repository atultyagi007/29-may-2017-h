...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 blade server using ESX 5.5 with 2 new compellent volumes to a new cluster and scale up a blade server.
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_before_104064_scaleup_bladeserver)
        self.getResources()
        networkType=inputForNetworkType.TestCase_104064_networkType
        if not networkType:
        ...
        if len(compellentList) == 0:
            ...
            self.log_data( 'Required no. of COMPELLENT not available')
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104064, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104064, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104064'
        deplyDesc=testCaseDescriptionMapping.TestCase_104064
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_TestCase_104064_after_scaleup_bladeserver)
        network_Type = inputForNetworkType.TestCase_104064_scaleUp_networkType
        if not network_Type:
        ...
        #scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_104064_after_scaleup_bladeserver)
