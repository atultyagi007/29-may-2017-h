...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 blade servers using ESX 5.1s with 2 compellent volume to a Existing cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_before_104073_scaleup_bladeserver)
        self.getResources()
        networkTypeServer1=inputForNetworkType.TestCase_104073_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_104073_networkType_server2
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104073, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104073, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104073'
        deplyDesc=testCaseDescriptionMapping.TestCase_104073
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_TestCase_104073_afte_scaleup_bladeserver)
        network_Type = inputForNetworkType.TestCase_104073_scaleUp_networkType
        if not network_Type:
        ...
        #scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_104073_afte_scaleup_bladeserver)
