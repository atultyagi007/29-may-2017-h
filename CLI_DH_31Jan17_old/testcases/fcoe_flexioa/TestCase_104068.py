...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 blade servers using ESX 5.5 with 2 compellent volume to an existing cluster and scale up a blade server
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_before_104068_scaleup_bladeserver)
        self.getResources()
        networkTypeServer1=inputForNetworkType.TestCase_104068_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_104068_networkType_server2
        ...
        if not networkTypeServer1:
            ...
            if networkTypeServer1=='Blade_2Port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_2port_MXL_FLEXIOA)
            elif networkTypeServer1=='Blade_4Port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_4port_MXL_FLEXIOA)
            elif networkTypeServer1=='Rack_2port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Rack_2port_MXL_FLEXIOA)
            elif networkTypeServer1=='Rack_4port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Rack_4port_MXL_FLEXIOA)
            else:
        ...
        if not networkTypeServer2:
            ...
            if networkTypeServer2=='Blade_2Port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_2port_MXL_FLEXIOA)
            elif networkTypeServer2=='Blade_4Port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_4port_MXL_FLEXIOA)
            elif networkTypeServer2=='Rack_2port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Rack_2port_MXL_FLEXIOA)
            elif networkTypeServer2=='Rack_4port_MXL_FLEXIOA':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Rack_4port_MXL_FLEXIOA)
            else:
                print " ERROR : missing network configuration for the test case "
                self.log_data( 'ERROR : missing network configuration for the test case ')
                return
        ...
        for device in compellentList:
            if device['ip']==globalVars.compellent_FCoE_IOA:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104068, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_104068, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104068'
        deplyDesc=testCaseDescriptionMapping.TestCase_104068
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_TestCase_104068_afte_scaleup_bladeserver)
        network_Type = inputForNetworkType.TestCase_104068_scaleUp_networkType
        if not network_Type:
            print " Please define the server and port type value in serverPoolValue parameter file for scaleup "
            self.log_data( 'Please define the server and port type value in inputForNetworkType parameter file for scaleup ')
            return
        if network_Type=='Blade_2Port_MXL_FLEXIOA':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_2port_MXL_FLEXIOA)
        elif network_Type=='Blade_4Port_MXL_FLEXIOA':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_4port_MXL_FLEXIOA)
        elif network_Type=='Rack_2port_MXL_FLEXIOA':
        ...
        #scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_104068_afte_scaleup_bladeserver)
