...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_126309_before_scaleup)
        self.getResources()
        
        networkTypeServer1=inputForNetworkType.TestCase_126309_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_126309_networkType_server2
        ...
        if not networkTypeServer1:
            ...
            if networkTypeServer1=='Blade_ISCSICOMP_COV_HYPERV_2PORT':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_2PORT)
            elif networkTypeServer1=='Blade_ISCSICOMP_COV_HYPERV_4PORT':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_4PORT)
            elif networkTypeServer1=='Rack_ISCSICOMP_COV_HYPERV_2PORT':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_2PORT)
            elif networkTypeServer1=='Rack_ISCSICOMP_COV_HYPERV_4PORT':
                payload=payload.replace("$networkConfiguration1",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_4PORT)
            else:
        ...
        if not networkTypeServer2:
            ...
            if networkTypeServer2=='Blade_ISCSICOMP_COV_HYPERV_2PORT':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_2PORT)
            elif networkTypeServer2=='Blade_ISCSICOMP_COV_HYPERV_4PORT':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_4PORT)
            elif networkTypeServer2=='Rack_ISCSICOMP_COV_HYPERV_2PORT':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_2PORT)
            elif networkTypeServer2=='Rack_ISCSICOMP_COV_HYPERV_4PORT':
                payload=payload.replace("$networkConfiguration2",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_4PORT)
            else:
                print " ERROR : missing network configuration for the test case "
                self.log_data( 'ERROR : missing network configuration for the test case ')
                return
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID), testCaseDescriptionMapping.TestCase_126309, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID),testCaseDescriptionMapping.TestCase_126309, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_126309'
        deplyDesc=testCaseDescriptionMapping.TestCase_126309
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        ...
        payload = self.readFile(globalVars.filename_TestCase_126309_afte_scaleup_CloneVM)
        network_Type = inputForNetworkType.TestCase_126309_scaleUp_networkType
        if not network_Type:
            ...
            return
        if network_Type=='Blade_ISCSICOMP_COV_HYPERV_2PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_2PORT)
        elif network_Type=='Blade_ISCSICOMP_COV_HYPERV_4PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_ISCSICOMP_COV_HYPERV_4PORT)
        elif network_Type=='Rack_ISCSICOMP_COV_HYPERV_2PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_2PORT)
        elif network_Type=='Rack_ISCSICOMP_COV_HYPERV_4PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_ISCSICOMP_COV_HYPERV_4PORT)
        else:
            print " ERROR : missing network configuration for the test case "
            self.log_data( 'ERROR : missing network configuration for the test case ')
            return
