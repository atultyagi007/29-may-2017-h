...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy basic VMware SD boot cluster on single FX2 server
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_VMware_Esxi)
        self.getResources()
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id),'Success','Template created  and published Successfully','Server : FX Server'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_VMware_Esxi'
        deplyDesc='Deploy basic VMware SD boot cluster on single FX2 server'
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        self.log_TestData(["", "", "",str(self.tc_Id),'Success','Template Deployed Successfully','Server : FX Server', "deploymentLogPath: %s"%deploymentLogPath])
                        ...
                        self.log_TestData(["", "", "",str(self.tc_Id),'Failed','Deployment Service Failed','Server : FX Server', "deploymentLogPath: %s"%deploymentLogPath  ])
