...
@author: raj.patel
Description:These test case covers these api.
/Authenticate   post
/Timezone/    get
/Timezone/    put
/NTP/      put
...
class Testcase(InitialTestBase): 
    ...
    def get_timezone(self):
        ...
        if state:
            ...
            test.log_TestData(["", "", "",str(test.tc_Id),'Success',"Successfully get TimeZone"])
        else:
    ...
    def delete_NTP(self):
        logger = self.getLoggerInstance()
        resRes,state=self.getResponse("DELETE", "NTP")
        print resRes
        logger.info(' Update NTP Response is ')
        logger.debug(resRes)
        if state:
            self.log_data( "NTP deleted, Response : %s"%str(resRes))
            test.log_TestData(["", "", "",str(test.tc_Id),'Success',"NTP deleted Successfully"])
        else:
            self.log_data("NTP not deleted.Error")  
        
        
    def get_NTP(self):
        logger = self.getLoggerInstance()
        resRes,state=self.getResponse("GET", "NTP")
        print resRes
        logger.info('NTP Response is ')
        logger.debug(resRes)
        if state:
            self.log_data( "NTP GET Response : %s"%str(resRes))
            test.log_TestData(["", "", "",str(test.tc_Id),'Success',"NTP get Successfully"])
        else:
            self.log_data("NTP not get.Error")  
