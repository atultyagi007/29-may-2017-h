...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    deploy a template with storage, server and then try to add cluster to a successful deployment
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_Testcase_before_102215_scaleup_Cluster)
        self.getResources()
        networkType=inputForNetworkType.TestCase_102215_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_102215, 'Success','Template created  and published Successfully','Server : Blade Server'])
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_102215, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_102215'
        deplyDesc=testCaseDescriptionMapping.TestCase_102215
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_102215_after_scaleup_Cluster)
