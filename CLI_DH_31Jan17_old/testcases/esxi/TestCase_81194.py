...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 servers with ESXI with 2 storage volumes,Mem Enable and a cluster with HA Enable
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_81194)
        self.getResources()
        
        networkTypeServer1=inputForNetworkType.TestCase_81194_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_81194_networkType_server2
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_81194, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_81194, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_81194'
        deplyDesc=testCaseDescriptionMapping.TestCase_81194
