...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    deploy a template with 1 storage volume and then try to add 2 volumes to a successful deployment
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_Testcase_before_102213_scaleup)
        self.getResources()   
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_102213, 'Success','Template created  and published Successfully','Server : Blade Server'])
            statausCreateTemplae= self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(testCaseID),testCaseDescriptionMapping.TestCase_102213, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_102213'
        deplyDesc=testCaseDescriptionMapping.TestCase_102213
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_102213_after_scaleup_2storage)
        ...
        if scaleUpResponse.status_code in (200, 201, 202, 203, 204):            
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        print "Deployment Status: %s"%resDS
                        self.log_TestData(["", "", "",str(testCaseID), deplyDesc, 'Success',' scaleUp Service Deployed Successfully', 'Server : Blade Server', "deploymentLogPath: %s"%deploymentLogPath])
