    ...
    def runTestCase(self):
        ...
        if status and resDevice > 0:
            for resInfo in resDevice:
                ...
                print tempId
                print resInfo
                componentType=resInfo['components'][0]['type']
                print componentType
                deviceId = "template"+"/"+tempId+"/"+componentType
                resDevice1,statusInfo=self.getResponse("GET", "Template",refId=deviceId)
                if statusInfo:
                    self.log_data("Result:",resDevice1)
                    self.log_data("Successfully tested /ServiceTemplate/template/{templateId}/{componentType} with GET action:")
                    self.log_TestData(["", "", "",str(self.tc_Id), 'Successfully tested /ServiceTemplate/template/{templateId}/{componentType} with GET action','Test case passed'])
                    return True
                else:
                    self.log_data("Test case Failed")
                    self.log_TestData(["", "", "",str(self.tc_Id), 'Test case failed for /ServiceTemplate/template/{templateId}/{componentType} with GET action', 'Failed','Test case failed'])
                    return False
