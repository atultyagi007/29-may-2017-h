...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    deploy a template with 1 storage volume and then try to add servers to a successful storage deployment
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_Testcase_before_102214_scaleup)
        self.getResources()
        networkType=inputForNetworkType.TestCase_102214_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_102214, 'Success','Template created  and published Successfully','Server : Blade Server'])
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_102214, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        deplyDesc=testCaseDescriptionMapping.TestCase_102214
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_TestCase_102214_after_scaleup_server)
        network_Type = inputForNetworkType.TestCase_102214_scaleUp_networkType
        if not network_Type:
