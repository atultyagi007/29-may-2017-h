        ...
        if status and resDevice > 0:
            for resInfo in resDevice:
                ...
                for deploymentDevice in deploymentDeviceList:
                    ...
                    resDevice1,statusInfo=self.getResponse("GET", "TemplateByDeviceId",refId=deviceId)
                    if statusInfo:
                        self.log_data("Result:",resDevice1)
                        self.log_data("Successfully tested /ServiceTemplate/device/{deviceId} with GET action:")
                        self.log_TestData(["", "", "",str(self.tc_Id), 'Successfully tested /ServiceTemplate/device/{deviceId} with GET action','Test case passed'])
                        return True
                    else:
                        self.log_data("Test case Failed")
                        self.log_TestData(["", "", "",str(self.tc_Id), 'Test case failed for /ServiceTemplate/device/{deviceId} with GET action', 'Failed','Test case failed'])
                        return False
