...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 servers with ESXI with 2 storage volumes and a cluster with HA Enable
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_81191)
        self.getResources()
        
        networkTypeServer1=inputForNetworkType.TestCase_81191_networkType
        networkTypeServer2=inputForNetworkType.TestCase_81191_networkType
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_81191, 'Success','Template created  and published Successfully','Server :2 Blade Servers'])
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_data( 'Failed to create/published template ')
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_81191,'Failed','Failed to create/published template'])
