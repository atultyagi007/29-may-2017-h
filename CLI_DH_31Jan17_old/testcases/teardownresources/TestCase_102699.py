...
'''
    # Test Case : delete servers, cluster and VM
    # Pre-Requisite : successful deployment has storage, servers, cluster, VM and application 
'''

class test_102699(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def getTearDownPayload(self,deploymentResponse):
        ...
        canSupAp.text = 'true'
        canSupStrg = root.find('canScaleupStorage')
        canSupStrg.text = 'true'
        
        for st in root.findall('serviceTemplate'):
            for cmpnt in st.findall('components'):
                if cmpnt.findtext('type')=='SERVER' or cmpnt.findtext('type')=='CLUSTER' or cmpnt.findtext('type')=='VIRTUALMACHINE':
