...
'''
    # Test Case : delete volume, server, cluster and vms from a successful deployment
    # Pre-Requisite : successful deployment has storage, servers, cluster, VM and application 
'''

class test_102701(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def getTearDownPayload(self,deploymentResponse):
        ...
        for st in root.findall('serviceTemplate'):
            for cmpnt in st.findall('components'):
                if cmpnt.findtext('type')=='SERVER' or cmpnt.findtext('type')=='STORAGE' or cmpnt.findtext('type')=='CLUSTER' or cmpnt.findtext('type')=='VIRTUALMACHINE':
