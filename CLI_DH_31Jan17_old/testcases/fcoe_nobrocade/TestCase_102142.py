...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 rack servers using ESX 5.5 with 2 compellent volume to a new cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_102142)
        self.getResources()
        networkType=inputForNetworkType.TestCase_102142_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_102142, 'Success','Template created  and published Successfully','Server : Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_102142, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_102142'
        deplyDesc=testCaseDescriptionMapping.TestCase_102142
