...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_102145)
        self.getResources()
        networkType=inputForNetworkType.TestCase_102145_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_102145, 'Success','Template created  and published Successfully','Server : Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_102145, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_102145'
        deplyDesc=testCaseDescriptionMapping.TestCase_102145
