...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 blade server using ESX 5.1 with 1 new compellent volume to an existing cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_104088)
        self.getResources()
        networkType=inputForNetworkType.TestCase_104088_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_104088, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_104088, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_104088'
        deplyDesc=testCaseDescriptionMapping.TestCase_104088
