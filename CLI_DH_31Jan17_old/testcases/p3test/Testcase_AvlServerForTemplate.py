...
@author: raj.patel
Description:Test case for Find available servers for template components
...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Test case for Find available servers for template components
    /Deployment/filter/{numOfDeployments}    post
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_ESXITemplate)
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            resInfo,status=self.getResponse("POST", "AvlServerForTemp",payload=tempPayload)
            self.log_data(status)
            self.log_data(resInfo)
            selectedServers=resInfo['selectedServers']
            
            if status :
                
                self.log_data("Available server list for template :%s"%str(len(selectedServers)))
                self.log_data("Successfully exported template and path:%s"%resInfo)
                self.log_data("Successfully tested api /Deployment/filter/{numOfDeployments}with post action")
                self.log_TestData(["", "", "",str(self.tc_Id), 'Successfully tested api /Deployment/filter/{numOfDeployments} with post action','Success','Test case passed'])
                return True
            else:
