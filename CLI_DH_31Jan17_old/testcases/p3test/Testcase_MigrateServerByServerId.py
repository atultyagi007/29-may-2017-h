...
@author: raj.patel
Description:Migrate Deployment based on deployment ID 
...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def preRunSetup(self):
        ...
        logger = self.getLoggerInstance()
        logger.debug('Cleaning server pool')
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_BFSMigrateServer)
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            if statausDeployedService:
                ...
                resInfo,status=self.migrateServer("PUT", "BFSMigrateServer",refId=refId)
                
                if status :
                    self.log_data(resInfo)
                    self.log_data("Successfully tested api /Deployment/migrate/{serviceId}/{serverId} with put action")
                    self.log_TestData(["", "", "",str(self.tc_Id), 'Successfully tested api /Deployment/migrate/{serviceId}/{serverId} with post action','Success','Test case passed'])
                    return True
                else:
    ...
    def test_publishTemplate(self,specificstandarduser=None):
        ...
        root = tree.getroot()
        ET.ElementTree(root).write(globalVars.publishedTemp_filename)
        DeplName = 'TestCase_BFSMigrateServer'
