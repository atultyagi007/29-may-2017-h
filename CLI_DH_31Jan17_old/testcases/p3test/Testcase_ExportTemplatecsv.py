...
class Testcase(DiscoverResourceTestBase):
    ...
    def runTestCase(self):
        ...
        resDevice,status1=self.runService("GET", "ExportcsvTemplate",responseType="file", requestContentType="file")
        print resDevice
        if status1:
            self.log_data("Exported Template in csv info")
            self.log_data(resDevice)
            self.log_TestData(["", "", "",str(self.tc_Id), 'Successfully tested /ServiceTemplate/export/csv with GET action:','Success','Successfully Test case passed'])
            return True 
        else:
