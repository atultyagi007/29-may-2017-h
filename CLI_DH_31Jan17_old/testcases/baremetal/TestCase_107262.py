...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Baremetal deployment on blade server with Centos7
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_107262)
        self.getResources()
        networkType=inputForNetworkType.TestCase_107262_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107262, 'Success','Template created  and published Successfully','Server :Blade Servers'])
            self.test_publishTemplate()
            
        else:
            self.log_data( 'Failed to create/published template ')
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107262, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_107262'
        deplyDesc=testCaseDescriptionMapping.TestCase_107262
