...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Baremetal deployment on Rack server with Centos7
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_107264)
        self.getResources()
        networkType=inputForNetworkType.TestCase_107264_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107264, 'Success','Template created  and published Successfully','Server :Rack Servers'])
            self.test_publishTemplate()
            
        else:
            self.log_data( 'Failed to create/published template ')
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107264, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_107264'
        deplyDesc=testCaseDescriptionMapping.TestCase_107264
