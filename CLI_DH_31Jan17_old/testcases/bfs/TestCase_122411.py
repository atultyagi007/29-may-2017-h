...
'''
    TestCase : Create template for BFS and deploy 1 server with Equallogic storage
'''
...
#             logger.debug(response)
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        logger = self.getLoggerInstance()
        payload = self.readFile(globalVars.filename_TestCase_122411)
        self.getResources()
        globalVars.testCaseFlowName ='exsi'
#       networkType = inputForNetworkType.TestCase_122411_networkType
#         if not networkType:
...
#             return
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID),testCaseDescriptionMapping.TestCase_122411, 'Success','Template created  and published Successfully','Server : Blade Server'])
            statausCreateTemplate= self.test_publishTemplate(publishedTemplateID)
            
        else:
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_122411, 'Failed','Failed to create/published template'])    
...
#         ET.ElementTree(root).write(globalVars.publishedTemp_filename, xml_declaration=False)
        DeplName='TestCase_122411'
        deplyDesc=testCaseDescriptionMapping.TestCase_122411
