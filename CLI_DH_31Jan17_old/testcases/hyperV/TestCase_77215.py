...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 M820 Blade servers with HyperV  non-R2 with 2 storage volumes and a cluster
    """ 
...
#     
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_77215)
        self.getResources()
        networkType=inputForNetworkType.TestCase_77215_networkType
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_77215, 'Success','Template created  and published Successfully','Server : M820 Blade server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_77215, 'Failed','Failed to create/published template'])
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        statausDeploy = True
                        self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Success','Template Deployed Successfully','Server : Blade Server', "deploymentLogPath: %s"%deploymentLogPath, deplyDesc])
...
#                         self.test_cleanePublishedTemplates()
                        ...
                        self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Failed','Deployment Service Failed','Server : Blade Server', "deploymentLogPath: %s"%deploymentLogPath, deplyDesc  ])
