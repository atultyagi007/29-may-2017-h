...
class Testcase(TemplateTestBase):
    """
    Deploy a rack server with HyperV on Windows 2012 non-R2 with 2 storage volumes and a cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_78806)
        self.log_data(" Going to Create Template :: ")
        self.getResources()
        networkType=inputForNetworkType.TestCase_78806_networkType
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_78806, 'Success','Template created  and published Successfully','Server: R620 Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_78806, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_78806'
        deplyDesc=testCaseDescriptionMapping.TestCase_78806
