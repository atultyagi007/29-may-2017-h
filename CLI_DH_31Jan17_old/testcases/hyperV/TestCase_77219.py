...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 2 M820 Blade servers with HyperV on Windows 2012 non-R2 with 2 storage volumes and a cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_77219)
        self.getResources()
        networkTypeServer1=inputForNetworkType.TestCase_77219_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_77219_networkType_server2
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_77219, 'Success','Template created  and published Successfully','Server :2 M820 Blade servers'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_77219, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_77219'
        deplyDesc=testCaseDescriptionMapping.TestCase_77219
        statausDeploy = False
