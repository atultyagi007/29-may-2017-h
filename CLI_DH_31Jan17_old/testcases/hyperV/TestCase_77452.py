...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 M620 Blade servers with HyperV  non-R2 with 2 storage volumes and a cluster with existing host name
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_77452)
        self.getResources()
        networkType=inputForNetworkType.TestCase_77452_networkType
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_77452, 'Success','Template created  and published Successfully','Server : M620 Blade server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_77452, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_77452'
        deplyDesc=testCaseDescriptionMapping.TestCase_77452
