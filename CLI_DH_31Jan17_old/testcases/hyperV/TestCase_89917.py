...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Add 1 Blade server to the deployment with compellent storage
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_89917)
        self.getResources()
        networkType=inputForNetworkType.TestCase_89917_networkType
        ...
        for device in compellentList:
            if device['ip']==globalVars.compellent_hyperV_blade:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_89917, 'Success','Template created  and published Successfully','Server : Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae =self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_89917, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_89917'
        deplyDesc=testCaseDescriptionMapping.TestCase_89917
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        ...
                        self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Failed','Deployment Service Failed',  "deploymentLogPath: %s"%deploymentLogPath])
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_ScaleUp_89917)
        network_Type = inputForNetworkType.TestCase_89917_scaleUp_networkType
        if not network_Type:
            ...
            return
        if network_Type=='RackHperV_2Port':
            payload=payload.replace("$networkConfiguration",networkConfiguration.RackHyperV_2Port)
        elif network_Type=='RackHperV_4Port':
            payload=payload.replace("$networkConfiguration",networkConfiguration.RackHyperV_4Port)
        elif network_Type=='BladeHyperV_2Port':
            payload=payload.replace("$networkConfiguration",networkConfiguration.BladeHyperV_2Port)
        elif network_Type=='BladeHyperV_4Port':
            payload=payload.replace("$networkConfiguration",networkConfiguration.BladeHyperV_4Port)
        else:
            print " ERROR : missing network configuration for the test case "
            self.log_data( 'ERROR : missing network configuration for the test case ')
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        ...
                        self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Success',' scaleUp Template Deployed Successfully', 'Server : Blade Server', "deploymentLogPath: %s"%deploymentLogPath])
...
#                         self.test_cleanePublishedTemplates()
                        ...
                        self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Failed','Deployment ScaleUped Service Failed','Server : Blade Server',  "deploymentLogPath: %s"%deploymentLogPath])
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), deplyDesc, 'Failed','Failed to deploy scaleUp Service'])
