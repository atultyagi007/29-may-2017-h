...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Baremetal deployment with Raid5 on H330 controller with hotspare2
    """ 
...
#         respCSP = self.cleanServerPool()
        ...
        servers = []
        serverType = serverPoolValue.TestCase_112196
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_112196)
        self.getResources()
        networkType=inputForNetworkType.TestCase_112196_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_112196, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_112196, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_112196'
        deplyDesc=testCaseDescriptionMapping.TestCase_112196
