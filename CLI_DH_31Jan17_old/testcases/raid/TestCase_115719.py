...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Baremetal deployment with Raid1,50 on H730 controller with hotspare1
    """ 
    ...
    def preRunSetup(self):
        ...
        servers = []
        serverType = serverPoolValue.TestCase_115719
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_115719)
        self.getResources()
        networkType=inputForNetworkType.TestCase_115719_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id),testCaseDescriptionMapping.TestCase_115719, deplyDesc,  'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(self.tc_Id),testCaseDescriptionMapping.TestCase_115719, deplyDesc, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_115719'
        deplyDesc=testCaseDescriptionMapping.TestCase_115719
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        ...
                        IPADDRESS=self.getServerIPAddress(deploymentRefId)
                        self.createExportXML(IPADDRESS)
                        self.log_data( 'Going to do RAID Validation ')
                        self.performRAIDValidation()
                        self.log_data( 'Going to call run WebServiceAPI to update the Qmetry ')
