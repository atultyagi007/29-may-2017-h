...
Associate the storage to the server component ; Select Advanced Raid Settings and Add virtual Disk for RAID1 with exactly 2 HDD�s.
BIOS Settings: Use the import from server button to import the BIOS and RAID settings from another server.
OS settings: Select ESxi 5.5 ; Enter an NTP server to set NTP on host; select auto generate hostname with a regular expression on model and number tag
Network settings: Select Fab1 A fabric
...
Server NDC correctly partitioned with appropriate networks associated
Vlans correctly tagged on IOM switch
Vcenter Datacenter create and cluster clreated
VM deployed on cluster.
Time set on VM and time synch is correct between appliance VM and server
Verify the correct BIOS settings and RAID settings are applied -�Success
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_WorkFlow4_Case1)
        self.getResources()
        ...
        for device in serverList:
            if device['ip']==inputForWorkFlowCases.ServerEntry_4:
                serverRefID1 = device['refid']
            if device['ip']==inputForWorkFlowCases.ServerEntry_5:
                serverRefID2 = device['refid']
        payload = payload.replace("$serversource",'manual').replace("$ServerEntry_4",serverRefID1).replace("$ServerEntry_5",serverRefID2)
    ...
    def test_publishTemplate(self):
        ...
        DeplName='WorkFlow4_Case1'
        deplyDesc='Deploy 2  servers with ESXI with 2 Equilogic storage volumes and a cluster with HA and DRS Enable and Red hat VM '
