...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
   Create and Publish a template that deploys 1 blade server (HyperV) with 2 new volumes to a SCVMM cluster. 
   Another admin logs in and kicks off the deployment and grants access to standard user2 -�
   Admin kicks off deployment of 1 server.

Admin scales up dup server on HyperV deployment -
    """ 
    ...
    def test_createAndPublishTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_WorkFlow6_Case2)
        self.getResources()
    ...
    def test_deployTemplate(self):
        ...
        DeplName='WorkFlow6Case2'
        deplyDesc='Create and Publish a template that deploys 1 blade server (HyperV) with 2 new volumes to a SCVMM cluster.'
...
#                         self.test_cleanDeployedTemplates(deploymentRefId)
    ...
    def test_scaleUpServerByAdmin(self, templateID, deploymentRefId, DeplName, deplyDesc):
        ...
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_WorkFlow6_Case2_hyperv_scaleup_server_adminuser)
