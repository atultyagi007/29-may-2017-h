...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
   ...
   Give access of the template to Standard user2. Admin deploys the service
   ...
   Fail both the servers.
   Verify that the migration occurs and new servers are selected for deployment.
   Verify that the deployment is successful after a successful migration.
   ...
   Verify that on tear down the service deleted and there are no exception files. The selected components are deleted.
    ...
    def test_createAndPublishTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_WorkFlow2_Case2)
        self.getResources()
        ...
        if inputForWorkFlowCases.serversource == "manual":
            ...
            for device in serverList:
                if device['ip']==inputForWorkFlowCases.ServerEntry_24:
                    serverRefID1 = device['refid']
                if device['ip']==inputForWorkFlowCases.ServerEntry_25:
                    serverRefID2 = device['refid']
                payload = payload.replace("$ServerEntry_24",serverRefID1).replace("$ServerEntry_25",serverRefID2)
    ...
    def test_deployTemplate(self):
        ...
        DeplName='WorkFlow2_Case2'
        deplyDesc='Deploy 2 Blade server with 2 new Volume to a SCCVM'
