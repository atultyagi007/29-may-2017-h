...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_WorkFlow1_Case2)
        self.getResources()
        ...
        if inputForWorkFlowCases.serversource == "manual":    
            ...
            for device in serverList:
                if device['ip']==inputForWorkFlowCases.ServerEntry_2:
                    serverRefID1 = device['refid']
                if device['ip']==inputForWorkFlowCases.ServerEntry_3:
                    serverRefID2 = device['refid']
            payload = payload.replace("$serversource",'manual').replace("$ServerEntry_2",serverRefID1).replace("$ServerEntry_3",serverRefID2)    
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc, refIdServer1 = None,refIdServer2 = None):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_WorkFlow1_Case2_scaleup)
        payload = payload.replace("$serversource",'manual').replace("$ServerEntry_2",refIdServer1).replace("$ServerEntry_3",refIdServer2)
