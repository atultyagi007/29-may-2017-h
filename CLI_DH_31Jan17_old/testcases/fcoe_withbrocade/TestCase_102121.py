...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 rack servers using ESX 5.5 with 2 compellent volume to a  Existing cluster
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_102121)
        self.getResources()
        networkType=inputForNetworkType.TestCase_102121_networkType
        if not networkType:
            ...
            if networkType=='Blade_2Port_FCOE':
                payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_2Port_FCOE)
            elif networkType=='Blade_4Port_FCOE':
                payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_4Port_FCOE)
            elif networkType=='Rack_2Port_FCOE':
                payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_2Port_FCOE)
            elif networkType=='Rack_4Port_FCOE':
                payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_4Port_FCOE)
            else:
        ...
        for device in compellentList:
            if device['ip']==globalVars.compellent_FCoE_withbrocade:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_102121, 'Success','Template created  and published Successfully','Server : Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_102121, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_102121'
        deplyDesc=testCaseDescriptionMapping.TestCase_102121
