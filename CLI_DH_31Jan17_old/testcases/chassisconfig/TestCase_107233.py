...
'''
'''
    This automation script covers three testcases
    TestCase 107233 : Add Chassis Name
    TestCase 107234 : Add Chassis DNS name
    TestCase 107235 : Add Chassis Location details
...
class TestCMC(TemplateTestBase,DiscoverResourceTestBase,UtilBase):
    ...
    def addChassisDetails(self):
        ...
        payload = payload.replace("$chassis_ID","ff8080814f2ad236014f2ad87e190034").replace("$chassis_comp_id",chassisServiceTag).replace("$chassis_cert_name",chassisPuppetName).replace("$chassis_name",chassisServiceTag).replace("$chassis_IP",chassisIP).replace("$chassis_user_value","[]").replace("$chassis_title_value",chassisPuppetName).replace("</ServiceTemplate>","")
        self.nameAndLocPayload = self.readFile(globalVars.Chassis_Addtnl_Params)
        self.nameAndLocPayload = self.nameAndLocPayload.replace("$chassisName",chassisConfigParam.chassisName).replace("$dnsName",chassisConfigParam.chassisDNSName).replace("$datacenter",chassisConfigParam.chassisDatacenterName).replace("$aisle",chassisConfigParam.chassisAisle).replace("$Rack",chassisConfigParam.chassisRack).replace("$Slot",chassisConfigParam.chassisRackSlot)
        payload = payload.replace("$chassisAdditionalParameters",self.nameAndLocPayload)
