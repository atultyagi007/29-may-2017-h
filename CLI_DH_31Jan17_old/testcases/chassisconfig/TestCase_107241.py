...
'''
    Test Case Description : Add physical port(s) to defined uplink port
'''
...
class TestCaseDefineUplink(TemplateTestBase,DiscoverResourceTestBase,UtilBase):
    ...
    def defineUplinks(self):
        ...
        uplinkDefinitions = uplinkDefinitions.replace("$uplinkId",chassisConfigParam.uplinkId).replace("$uplinkName",chassisConfigParam.uplinkName).replace("$Workload", templateInputReqValueHyperV.workloadID).replace("$PXE", templateInputReqValueHyperV.pXEID).replace("$HypervisorManagement", templateInputReqValueHyperV.hypervisorManagementID).replace("$VMotion", templateInputReqValueHyperV.vMotionID).replace("$ClusterPrivate", templateInputReqValueHyperV.clusterPrivateID).replace("$ISCSI", templateInputReqValueHyperV.iSCSIID).replace("$portChannel",chassisConfigParam.portChannelNumber)
        ...
        uplinkValues= '''["$uplinkId"]'''
        uplinkValues = uplinkValues.replace("$uplinkId",chassisConfigParam.uplinkId)
        UpLinkIDValue = '''{"uplinkId":"$uplinkId","uplinkName":"$uplinkName","portChannel":"$portChannel","portMembers":["Te
                    0/53","Te 0/43","Te
                    0/37"],"portNetworks":["$Workload","$VMotion","$HypervisorManagement","$PXE","$ClusterPrivate","$ISCSI"]}'''
        UpLinkIDValue = UpLinkIDValue.replace("$uplinkId",chassisConfigParam.uplinkId).replace("$uplinkName",chassisConfigParam.uplinkName).replace("$Workload", templateInputReqValueHyperV.workloadID).replace("$PXE", templateInputReqValueHyperV.pXEID).replace("$HypervisorManagement", templateInputReqValueHyperV.hypervisorManagementID).replace("$VMotion", templateInputReqValueHyperV.vMotionID).replace("$ClusterPrivate", templateInputReqValueHyperV.clusterPrivateID).replace("$ISCSI", templateInputReqValueHyperV.iSCSIID).replace("$portChannel",chassisConfigParam.portChannelNumber)
        for iomDetail in iomList:
            payloadIOM = ""
            payloadIOM = self.readFile(globalVars.Configure_iom_Uplink)
            payloadIOM = payloadIOM.replace("$switch_ID", iomDetail['puppetCertName']).replace("$switch_Component_ID",iomDetail['serviceTag']).replace("$switch_Cert_Name",iomDetail['puppetCertName']).replace("$switch_name",iomDetail['serviceTag']).replace("$switch_IP",iomDetail['IP']).replace("$switch_title_value", iomDetail['puppetCertName']).replace("$vltEnabled",chassisConfigParam.vltEnabled).replace("$quadPortMode",chassisConfigParam.quadPortMode)
            IOAddnlPayload = self.readFile(globalVars.Uplink_Addtnl_Params)
            IOAddnlPayload = IOAddnlPayload.replace("$uplinkId",chassisConfigParam.uplinkId).replace("$uplinkValues",uplinkValues).replace("$UpLinkIDValue",UpLinkIDValue)
            payloadIOM = payloadIOM.replace("$switchAdditionalParameters",IOAddnlPayload)
