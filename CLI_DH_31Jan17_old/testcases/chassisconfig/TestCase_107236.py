...
'''
        107236 : Add iDrac DNS name (all server(s))
...
class TestIdracDNS(TemplateTestBase,DiscoverResourceTestBase,UtilBase):
    ...
    def addIdracDNS(self):
        ...
        for serverDetail in serverList:
            ...
            payloadServer = payloadServer.replace("$ID",serverDetail['ID']).replace("$server_componentID",serverDetail['serviceTag']).replace("$server_Cert_Name",serverDetail['puppetCertName']).replace("$server_name",serverDetail['serviceTag']).replace("$server_ipAddress",serverDetail['IP']).replace("$server_title_Value",serverDetail['puppetCertName']).replace("<value>$server_user_Value</value>","").replace("$chassisAdditionalParameters","")
            serverAddnlPayload = self.readFile(globalVars.Server_Addtnl_Params)
            serverAddnlPayload = serverAddnlPayload.replace("$server_dns_name",chassisConfigParam.serverDNSName)
            payloadServer = payloadServer.replace("$serverAdditionalParameters",serverAddnlPayload)
