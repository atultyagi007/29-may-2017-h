...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Using FX2 Servers as rack servers for ESXi deployment
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_110628)
        self.getResources()
        networkType=inputForNetworkType.TestCase_110628_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id),testCaseDescriptionMapping.TestCase_110628, 'Success','Template created  and published Successfully','Server : Rack Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_110628, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_110628'
        deplyDesc= testCaseDescriptionMapping.TestCase_110628
