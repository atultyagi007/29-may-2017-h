...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    2 server ESX end to end deployment with EQL
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_110632)
        self.getResources()
        
        networkTypeServer1=inputForNetworkType.TestCase_110632_networkType_server1
        networkTypeServer2=inputForNetworkType.TestCase_110632_networkType_server2
        ...
        if not networkTypeServer1:
            ...
            return
        if networkTypeServer1=='Blade_Fx2_2PORT':
            payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_Fx2_2PORT)
        elif networkTypeServer1=='Blade_Fx2_4PORT':
            payload=payload.replace("$networkConfiguration1",networkConfiguration.Blade_Fx2_4PORT)
        else:
        ...
        if not networkTypeServer2:
            ...
            self.log_data( 'Please define the server and port type value in inputForNetworkType parameter file for this test case ')
            return
        if networkTypeServer2=='Blade_Fx2_2PORT':
            payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_Fx2_2PORT)
        elif networkTypeServer2=='Blade_Fx2_4PORT':
            payload=payload.replace("$networkConfiguration2",networkConfiguration.Blade_Fx2_4PORT)
        else:
            print " ERROR : missing network configuration for the test case "
            self.log_data( 'ERROR : missing network configuration for the test case ')
            return
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_110632, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_110632, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_110632'
        deplyDesc=testCaseDescriptionMapping.TestCase_110632
