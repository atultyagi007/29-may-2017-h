...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 server deployment with storage scaleup
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_110635_before_scaleup_storage)
        
        self.getResources()
        networkType=inputForNetworkType.TestCase_110635_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_110635, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(self.tc_Id), testCaseDescriptionMapping.TestCase_110635, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_110635'
        deplyDesc=testCaseDescriptionMapping.TestCase_110635
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        scaleUpResponse = self.scaleUpdeployEsxiTemplate(templateID, deploymentRefId, DeplName, deplyDesc, globalVars.filename_TestCase_110635_after_scaleup_storage)
...
#                         
            ...
            self.log_TestData(["", "", "",str(testCaseID), deplyDesc, 'Failed','Scaleup Deployment Service Failed'])
