...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    FC - On a successful VMWare deployment, scale up a network.    
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_53302)
        self.getResources()
        networkType=inputForNetworkType.TestCase_53302_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_53302, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_53302, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_53302'
        deplyDesc=testCaseDescriptionMapping.TestCase_53302
