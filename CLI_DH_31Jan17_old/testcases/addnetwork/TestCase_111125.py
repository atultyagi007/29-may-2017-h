...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    ...
    '''
        Add network to vm on a successful hyperV deployment
    '''
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_53278_before_scaleup_application)
        self.getResources()
        statausCreateTemplae = False
        networkType=inputForNetworkType.TestCase_53278_networkType
        if not networkType:
        ...
        print " STORAGE RES : %s"%(self.storageRes)
        if len(self.storageRes) == 0:
            print "Required no. of Storage not available"
        else:
            storageId = self.storageRes[0]["refid"]
        print " STORAGE ID : %s"%str(storageId)
        
        templateResponse= self.createTemplate(payload,storageId,scvmmId=None) 
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID),   testCaseDescriptionMapping.TestCase_111125, 'Success','Template created  and published Successfully','Server : Blade Server'])
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(testCaseID),   testCaseDescriptionMapping.TestCase_111125, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_111125'
        deplyDesc=testCaseDescriptionMapping.TestCase_111125
