...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Add network to hosts on a successful ESXi deployment
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_111118)
        self.getResources()
        statausCreateTemplae = False
        networkType=inputForNetworkType.TestCase_111118_networkType
        if not networkType:
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_111118'
        deplyDesc='Add network to hosts on a successful ESXi deployment'
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        print "Deployment Status: %s"%resDS
                        break
                        self.log_TestData(["", "", "", str(self.tc_Id),'Success',' scaleUp Network Done Successfully', 'Server : Blade Server', "deploymentLogPath: %s"%deploymentLogPath])
