...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 server  with ESXI with 2 storage volumes and a cluster with HA and DRS Enable
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_53338)
        self.getResources()
        networkType=inputForNetworkType.TestCase_53338_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_53338, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_53338, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_53338'
        deplyDesc=testCaseDescriptionMapping.TestCase_53338
