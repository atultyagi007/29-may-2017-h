...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Deploy 1 server, 1 storage, 1 cluster and 1 VM after that scale up with one server
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_Testcase_53293_before_scaleup)
        self.getResources()
        statausCreateTemplae = False
        networkType=inputForNetworkType.TestCase_53293_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "", str(testCaseID),  testCaseDescriptionMapping.TestCase_53293, 'Success','Template created  and published Successfully','Server : Blade Server'])
            self.log_data( 'Successfully created  and published Template ')
            statausCreateTemplae = self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "", str(testCaseID),  testCaseDescriptionMapping.TestCase_53293, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        statausDeploy = False
        DeplName='TestCase_53293'
        deplyDesc=testCaseDescriptionMapping.TestCase_53293
    ...
    def test_scaleUpTemplate(self, templateID, deploymentRefId, DeplName, deplyDesc):
        logger = self.getLoggerInstance()
        testCaseID=self.getTestCaseID(__file__)
        payload = self.readFile(globalVars.filename_TestCase_53293_scaleup_server)
        network_Type = inputForNetworkType.TestCase_53293_scaleUp_networkType
        if not network_Type:
            ...
            return
        if network_Type=='Rack_Esxi_FC_2PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_Esxi_FC_2PORT)
        elif network_Type=='Rack_Esxi_FC_4PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Rack_Esxi_FC_4PORT)
        elif network_Type=='Blade_Esxi_FC_2PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_FC_Esxi_2PORT)
        elif network_Type=='Blade_Esxi_FC_4PORT':
            payload=payload.replace("$networkConfiguration",networkConfiguration.Blade_Esxi_FC_4PORT)
        else:
            print " ERROR : missing network configuration for the test case "
            self.log_data( 'ERROR : missing network configuration for the test case ')
