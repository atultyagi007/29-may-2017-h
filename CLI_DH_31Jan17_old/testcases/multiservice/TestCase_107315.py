...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    Multiple service deployments on blades/rack using num tag
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_107315)
        self.getResources()
        networkType=inputForNetworkType.TestCase_107315_networkType
        if not networkType:
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107315, 'Success','Template created  and published Successfully','Server :2 Blade Servers'])
            self.test_publishTemplate()
            
        else:
            self.log_data( 'Failed to create/published template ')
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107315, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_107315'
        deplyDesc=testCaseDescriptionMapping.TestCase_107315
