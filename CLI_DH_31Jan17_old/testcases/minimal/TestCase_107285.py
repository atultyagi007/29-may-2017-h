...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
    C series deployment with Centos7
    """ 
    ...
    def test_createTemplate(self):
        ...
        self.setNetworkList()
        payload = self.readFile(globalVars.filename_TestCase_107285)
        self.getResources()
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(self.tc_Id), testCaseDescriptionMapping.TestCase_107285, 'Success','Template created  and published Successfully','Server : C series Servers'])
            self.test_publishTemplate()
            
        else:
            self.log_data( 'Failed to create/published template ')
            self.log_TestData(["", "", "",str(self.tc_Id),testCaseDescriptionMapping.TestCase_107285, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_107285'
        deplyDesc=testCaseDescriptionMapping.TestCase_107285
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        ...
                        self.log_TestData(["", "", "",str(self.tc_Id), deplyDesc, 'Failed','Deployment Service Failed','Server : C series Server', "deploymentLogPath: %s"%deploymentLogPath  ])
