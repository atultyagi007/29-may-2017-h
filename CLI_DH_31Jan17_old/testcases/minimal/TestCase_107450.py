...
class Testcase(TemplateTestBase,DiscoverResourceTestBase):
    """
Scale up server component on deployment on C series server using minimal server component    
    """ 
    ...
    def test_createTemplate(self):
        ...
        payload = self.readFile(globalVars.filename_TestCase_107450_before_scaleup_CSeriesserver)
        self.getResources()
        ...
        if templateResponse.status_code in (200, 201, 202, 203, 204):
            ...
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_107450, 'Success','Template created  and published Successfully','Server : C series  Server'])
            self.test_publishTemplate()
            
        else:
            self.log_TestData(["", "", "",str(testCaseID), testCaseDescriptionMapping.TestCase_107450, 'Failed','Failed to create/published template'])
    ...
    def test_publishTemplate(self):
        ...
        DeplName='TestCase_107450'
        deplyDesc=testCaseDescriptionMapping.TestCase_107450
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        ...
                        self.log_TestData(["", "", "",str(self.tc_Id),  deplyDesc, 'Success','Template Deployed Successfully','Server : C series Server', "deploymentLogPath: %s"%deploymentLogPath])
                        self.log_data( 'Successfully Deployed Service for the Deployment Name : %s'%DeplName)
                        break
                    else:
                        print "Deployment Status: %s"%resDS
                        self.log_TestData(["", "", "",str(testCaseID),  deplyDesc, 'Failed','Deployment Service Failed','Server : C series Server',  "deploymentLogPath: %s"%deploymentLogPath])
        ...
        if scaleUpResponse.status_code in (200, 201, 202, 203, 204):            
            ...
            while loop:
                ...
                if resDS.lower() in ("in_progress"):
                    ...
                    if resDS.lower() in ("complete"):
                        print "Deployment Status: %s"%resDS
                        self.log_TestData(["", "", "",str(testCaseID), deplyDesc, 'Success',' scaleUp Template Deployed Successfully', 'Server : C series Server', "deploymentLogPath: %s"%deploymentLogPath])
...
#                         self.test_cleanePublishedTemplates()
                        ...
                        print "Deployment Status: %s"%resDS
                        self.log_TestData(["", "", "",str(testCaseID), deplyDesc, 'Failed','Deployment ScaleUped Service Failed','Server : C series Server',  "deploymentLogPath: %s"%deploymentLogPath])
