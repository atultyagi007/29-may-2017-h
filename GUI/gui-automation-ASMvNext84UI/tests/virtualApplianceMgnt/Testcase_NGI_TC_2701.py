...
Description: Enable DHCP setting  with same start and end IP and do a deployment.
Test Flow    : 1) Log into the ASM UI and navigate to the Virtual Appliance Management page, 
              2) Enable DHCP setting with same start and end IP
              3) Do a Deployment
...
class Testcase(Manager.Manager): 
    """
     Enable DHCP setting with same start and end IP and do a deployment.
    ...
    def runTestCase(self):
        ...
        dns= globalVars.DHCP_Dns
        domain= globalVars.DHCP_Domain 
