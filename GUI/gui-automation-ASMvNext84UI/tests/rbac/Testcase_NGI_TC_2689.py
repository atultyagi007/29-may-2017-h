...
Description: Verify that if the Admin assigns specific users while creating template without creating any user, there 
        is an error message and no failure
'''
...
class Testcase(Manager.Manager): 
    """
    Assign specific users while creating template without creating any user
    """
    ...
    def test_functionality(self):        
        ...
        self.createSampleTemplate(templateName=templateName, deleteAndCreate=True, publishedTemplate=True, 
                                  userList=[globalVars.standardUser], passExpectation=True)
