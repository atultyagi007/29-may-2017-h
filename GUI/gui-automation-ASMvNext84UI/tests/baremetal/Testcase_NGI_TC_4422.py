...
Created Date: Aug 03, 2016
Description: BAREMETAL LINUX deployment with PXE and Static Private Workload on same port and another workload at other port.
'''
...
class Testcase(Manager.Manager): 
    """
    BAREMETAL LINUX deployment with PXE and Static Private Workload on same port and another workload at other port.
    """
    ...
    def preRunSetup(self):
        ...
        fileName = "BAREMETAL_LINUX_4422.json"      
...
#         
    ...
    def runTestCase(self):
        ...
        #=======================================================================
        expectedTaggedVlans=[]
        expectedUntaggedVlans=[]
        taggedVlans, untaggedVlans= self.getVLanTags(self.serviceName)
        networkConfig = utility.readExcel(globalVars.inputFile, "Network")
        for network in networkConfig:
            if network["Name"]=="autoPXE":
                expectedUntaggedVlans.append(int(network["VLANID"]))
            elif network["Name"]=="autoPublicLAN":
                expectedTaggedVlans.append(int(network["VLANID"]))
        if (expectedTaggedVlans in taggedVlans) and (expectedUntaggedVlans in untaggedVlans):
            self.succeed("Correct Network tagging on successful deployment")
        else:
            self.failure("Incorrect Network tagging on deployment", raiseExc= True)
