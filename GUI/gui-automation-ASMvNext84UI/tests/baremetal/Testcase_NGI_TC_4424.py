...
Description: BAREMETAL LINUX deployment with two Public DHCP workload on Port2 and PXE on Port1.
'''
...
class Testcase(Manager.Manager): 
    """
    BAREMETAL LINUX deployment with two Public DHCP workload on Port2 and PXE on Port1.
    """
    ...
    def preRunSetup(self):
        ...
        fileName = "BAREMETAL_LINUX_4424.json"      
...
#             self.createServerPool(self.components)
        ...
        if len(result) > 0:
            if result[0]["Status"] in ("Success"):
                self.failure("Deployed Service '%s' with 2 workload on same port"%self.serviceName, raiseExc= True)
            else:
                self.succeed("Failed to Deploy Service with 2 workload on same port'%s'"%self.serviceName)
        else:
