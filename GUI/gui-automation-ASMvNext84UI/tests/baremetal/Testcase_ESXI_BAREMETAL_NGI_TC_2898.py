...
Description: Baremetal deployment with ESXi.
Covered test cases 2898,2900.
'''
...
class Testcase(Manager.Manager): 
    """
    Baremetal deployment with ESXi
    """
...
#         fileName = globalVars.jsonMap["bareMetalexsi"]
        fileName = "BAREMETAL_LINUX_2898_2900.json"        
...
#             self.failure("Failed to Deploy Service '%s' in expected time"%self.serviceName, raiseExc=True)        
        ...
        self.ipList=self.serverIPList(self.serviceName)
        # Scale down service
        
        self.scaleDownService(self.serviceName, self.components)
        
        self.deleteServiceAfterScaleDown(self.serviceName)
        #Server state after tear down in Resources page
        self.validateServerStateAfterTearDown(self.ipList)
        #Server state after tear down on Server console
        self.serverConsoleStateAfterTearDown(self.ipList)
