...
Test Flow   :Login as a Admin user and Create user bundle for MXL switch.
'''
...
class Testcase(Manager.Manager): 
    ...
    def runTestCase(self):        
        ...
        ComplianceStatusPreBundleAdd=self.getReourceModelComplainceStatus("Switches","MXL-10/40GbE")
        if ComplianceStatusPreBundleAdd=="Compliant":
            self.succeed("need not add bundle, switches already in compliance state")
        else:
            self.addSwitch_CustomBundles("MXL-10/40GbE")
            time.sleep(10)
