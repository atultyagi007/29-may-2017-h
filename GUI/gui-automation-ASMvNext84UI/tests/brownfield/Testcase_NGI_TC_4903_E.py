...
@author: raj.patel
Description:On the "Summary" page, verify that all entries including mouseovers are correct.(cluster is yes, servers are unmanaged, and storage is not in inventory and attached servers are unmanaged.
...
class Testcase(Manager.Manager): 
    """
     Brownfield - cluster is yes, servers are unmanaged, and storage is not in inventory and attached servers are unmanaged
    """
...
#                 self.discoverResource_Type("Chassis", resourceIP, resourceIP, "Managed","Dell PowerEdge iDRAC Default", "", "Dell chassis default", "","Dell switch default", "")
    ...
    def runTestCase(self):
        ...
        self.brownFieldServiceSchanario(self.brownFieldServiceName,self.brownFieldCompName,self.targetVirtualMachineServiceTag,self.dataCenterName,self.clusterName,serverDiscov=self.serverDiscov,serverState=self.serverState)
