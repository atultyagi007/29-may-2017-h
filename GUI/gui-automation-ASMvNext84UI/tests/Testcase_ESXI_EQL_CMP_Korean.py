'''
Description: Deploy ESXi Compellent FC Diverged workflow with VDS Enabled for Chinese
'''
...
class Testcase(Manager.Manager): 
    ...
    def preRunSetup(self):
        ...
        fileName = globalVars.jsonMap["esxicpl_chinese"]  
        try:   
            jsonResult = self.getTemplateConfiguration(fileName) 
            self.language="Korean"
            translatedJson = self.translateJSONValue(jsonResult,self.language)
            self.components = translatedJson
...
#             self.createServerPool(self.components)
        ...
        if len(result) > 0:
            if result[0]["Status"] in ("Success"):
                self.succeed("Successfully Deployed Service")
            else:
                self.failure("Failed to Deploy Service", raiseExc=True)
        else:
