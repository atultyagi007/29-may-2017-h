...
Test Flow   :Login as a Admin user and Create user bundle for FX2 switch.
'''
...
class Testcase(Manager.Manager): 
    ...
    def runTestCase(self):        
        ...
        ComplianceStatusPreBundleAdd=self.getReourceModelComplainceStatus("Switches","PE-FN-2210S-IOM")
        if ComplianceStatusPreBundleAdd=="Compliant":
            self.succeed("need not add bundle, switches already in compliance state")
        else:
            self.addSwitch_CustomBundles("PE-FN-2210S-IOM")
            time.sleep(10)
