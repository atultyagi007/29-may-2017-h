...
Description : Create user bundle for Switch S6000
Test Flow   :Login as a Admin user and Create user bundle for switch S6000
'''
...
class Testcase(Manager.Manager): 
    ...
    def runTestCase(self):        
        ...
        ComplianceStatusPreBundleAdd=self.getReourceModelComplainceStatus("Switches","S6000")
        if ComplianceStatusPreBundleAdd=="Compliant":
            self.succeed("need not add bundle, switches already in compliance state")
        else:
            self.addSwitch_CustomBundles("S6000")
            time.sleep(10)
