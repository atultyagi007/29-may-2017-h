...
Description: Add an EMC storage device, Resource State = 'reserved'. Device info will be read from config/Input.xlsx.
'''
...
#        self.deleteCredential(credential["Name"], True)
        ...
        originalResourceState = resource["Resource State"]
        resource["Resource State"] = "Reserved"
        ...
        if status:
            ...
            if status:
                ...
                if status:
                    ...
                    globalVars.browserObject, status, result = pageObject.getActivityMessage(globalVars.deviceId)
        ...
        status, result = self.selectResource(globalVars.deviceId)
        if not status:
            self.failure("Failed to find resource {}, result :: {}!".format(globalVars.deviceId), result, raiseExc=True)

        status, result = self.selectResourceState(originalResourceState)
        if not status:
            self.failure("Failed to find resource {}, result :: {}!".format(globalVars.deviceId), result, raiseExc=True)
