...
class Testcase(Manager.Manager): 
    ...
    def runTestCase(self):
        ...
        if len(columnMismatch) > 0:
            if len(columnMismatch) == 3 and "Managed State" and "Firmware" and "Deployment Status" in columnMismatch:
...
#                     found = True
#                     for key in resource.keys():
#                         if key in ("OS Hostname", "IP Address","Deployment Status"):
#                             continue
#                         elif key == "State":
...
#                                 break
#                         else:
#                             if resource[key].strip().replace("-","") != resourcecsv[key].strip():
#                                 failList.append("Mismatch in row CSV :: %s -> UI :: %s"%(str(resourcecsv), str(resource)))
#                                 break
#             if not found:
#                 failList.append("Mismatch in row CSV :: %s -> UI :: %s"%(str(resourcecsv), str(resource)))
